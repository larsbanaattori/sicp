...
(define (raise-package)
  ...
  (put 'raise 'rational (lambda (x) (make-real (/ (rat-numer (contents x)) (rat-denom (contents x))))))
...
(define (project-package)
  ...
  (define rat-numer (get 'numer 'rational))
  ; Interface to rest of system
  (put 'project 'complex (lambda (x) (make-real (real-part (contents x)))))
...
(define (eq-package)
  ...
  (put 'eq '(rational rational)
       (lambda (x y)
         (let ((x (contents x))
               (y (contents y)))
           (and (= (rat-numer x) (rat-numer y)) (= (rat-denom x) (rat-denom y))))))
...
(define (eq-generic x y)
  ((get 'eq (list (type-tag x) (type-tag y))) x y))
